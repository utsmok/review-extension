...
function allHighScoringEvaluations(): Evaluation[] {
  const scoringIds = [
    "TR.data_source_clarity", "TR.methodology_disclosure",
    "RE.accuracy_and_hallucination", "RE.variance_consistency",
    "US.workflow_integration", "US.cognitive_guardrails",
    "SE.algorithmic_fairness", "SE.data_handling",
    "TC.source_attribution_depth", "TC.bibliometric_credibility",
  ];
  return [
...
describe("buildNutritionLabel", () => {
  ...
  it("contains the verdict text", async () => {
    const html = await buildNutritionLabel(makeMetadata(), [], RUBRIC);
    // With no evaluations and no finalization → NOT EVALUATED
